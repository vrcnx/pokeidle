...
function ReportBugDialog() {
  ...
  const submit = async (e: React.FormEvent) => {
    e.preventDefault();
    setError(null);
    if (title.trim().length < 3) { setError("Title is too short."); return; }
    if (description.trim().length < 10) { setError("Please describe the bug a bit more (10+ chars)."); return; }
    setBusy(true);
    try {
      ...
      if (err instanceof ApiError) {
        if (err.status === 429) {
          setError("You've sent a few reports recently — try again in an hour.");
        } else {
  ...
  return (
    ...
    >
      <header className="g-modal-head">
        <h2>Report a bug</h2>
        <button className="g-modal-close" onClick={closeReportBug} aria-label="Close">×</button>
      </header>
      ...
      <div className="g-modal-body">
        {submitted ? (
          <section className="g-card g-card-full">
            <h3>Thanks!</h3>
            <p>We've got it. The admin team will look at this — no need to repost.</p>
          </section>
        ) : (
          <form onSubmit={submit} className="report-bug-form">
            <section className="g-card g-card-full">
              <p className="g-help" style={{ marginTop: 0 }}>
                Tell us what went wrong. We'll automatically include your
                current URL, browser, and a snapshot of your party + last
                few battle log lines so the admin can reproduce.
              </p>
              <label className="auth-label">
                ...
                <input
                  ...
                  maxLength={120}
                  autoFocus
                  placeholder="Short summary (e.g. 'Trade animation freezes mid-swap')"
                />
              </label>
              <label className="auth-label">
                ...
                <textarea
                  ...
                  maxLength={4000}
                  rows={6}
                  placeholder={"Steps to reproduce — what did you do, what happened, what did you expect?"}
                />
              </label>
      ...
      <footer className="g-modal-foot">
        {submitted ? (
          <>
            <span style={{ flex: 1 }} />
            <button className="g-btn-primary" onClick={closeReportBug}>Close</button>
          </>
        ) : (
          <>
            <button className="g-btn-ghost" onClick={closeReportBug}>Cancel</button>
            <span style={{ flex: 1 }} />
            <button
