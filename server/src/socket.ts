...
export function attachSocketServer(httpServer: HttpServer): Server {
  ...
  io.on("connection", async (socket) => {
    ...
    // ── Shiny catches: REMOVED ───────────────────────────────────────
    // Every shiny catch used to post to global chat. It worked exactly as
    // designed and that was the problem: shinies are common enough across a
    // whole playerbase that the feed became a wall of "X caught a shiny Y!"
    // cards with real conversation scrolling past behind them.
    //
    // The client no longer emits `shiny:announce` and nothing here listens
    // for it, so a stale client shouting into the void is simply ignored.
    // The `kind: "shiny"` rows already in the database still render — see
    // utils/systemChatCards.ts — because deleting the renderer would turn
    // months of history into unattributed plain text.
